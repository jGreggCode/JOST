	...
	if(isset($_POST['itemDetailsItemNumber'])){
		...
		if(!empty($itemNumber) && !empty($itemName) && isset($quantity) && isset($unitPrice)){
			...
			if($stockStatement->rowCount() > 0){
				...
				$insertItemSql = 'INSERT INTO item(itemNumber, category, itemName, discount, stock, unitPrice, status, description) VALUES(:itemNumber, :category, :itemName, :discount, :stock, :unitPrice, :status, :description)';
