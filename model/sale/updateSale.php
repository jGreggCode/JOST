	...
	if(isset($_POST['saleDetailsSaleID'])){
		...
		if(isset($saleDetailsItemNumber) && isset($saleDetailsSaleDate) && isset($saleDetailsQuantity) && isset($saleDetailsUnitPrice) && isset($saleDetailsCustomerID)){
			...
			if ($_SESSION['usertype'] != 'Admin') {
				echo '<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button>Only admin can update the sales information.</div>';
				exit();
			}
			...
			if($originalSaleQuantityStatement->rowCount() > 0){
				...
				if($originalOrderItemNumber !== $saleDetailsItemNumber) {
					...
					$updateSaleDetailsSql = 'UPDATE sale SET itemNumber = :itemNumber, saleDate = :saleDate, itemName = :itemName, unitPrice = :unitPrice, discount = :discount, quantity = :quantity, customerName = :customerName, customerID = :customerID, `status` = :status WHERE saleID = :saleID';
