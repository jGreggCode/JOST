...
// File that creates the audit details search table
auditDetailsSearchTableCreatorFile = 'model/audit/auditDetailsSearchTableCreator.php';
...
$(document).ready(function(){
	...
	searchTableCreator('auditDetailsTableDiv', auditDetailsSearchTableCreatorFile, 'auditDetailsTable');
							...
							columns: [
								{
									...
									alignment: 'left',
									margin: [20, 10, 0, 0] // margin: [left, top, right, bottom]
								}
							]
						};
					});
...
function getPurchaseDetailsToPopulate(){
	...
	$.ajax({
		...
		success: function(data){
			...
			$('#purchaseDetailsUnitPrice').val(data.costing);
...
function getSaleDetailsToPopulate() {
    ...
    $.ajax({
        ...
        success: function(data) {
			...
			$('#saleDetailsCash').val(data.payment);
            $('#saleDetailsUnitPrice').val(data.unitPrice);
            $('#saleDetailsItemStatus').val(data.status).trigger("chosen:updated");
        },
        complete: function() {
            calculateTotalInSaleTab();
            getItemStockToPopulate('saleDetailsItemNumber', getItemStockFile, 'saleDetailsTotalStock');
